...
export type ResizableSketchProps<Props extends SketchProps = SketchProps> = Props & {
...
export default function ResizableP5Sketch({ sketch, sketchProps }: ResizableP5Props) {
  ...
  const { ref, width = undefined, height = undefined } = useResizeObserver<HTMLDivElement>();
  ...
  return (
    <div ref={ref} className="h-[60vh] aspect-auto">
      {isDisplayable && <NextReactP5Wrapper sketch={sketch} parentWidth={width} parentHeight={height} {...sketchProps} />}
    </div>
  )
}